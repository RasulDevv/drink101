...
data.forEach((product, index) => {
    box.insertAdjacentHTML('beforeend', `
        <a href="#products/${product.id}" id="product${product.id}" data-index="${index}" class="link">
...
document.querySelectorAll('.link').forEach((link, productIndex) => {
    link.onclick = function(e) {
        ...
        document.querySelector(`.${this.getAttribute('id')} > h3 > span`).innerHTML = products[productIndex].HighSumm
        ...
        inputsOfBulk.forEach((input, radioIndex) => {
            ...
            input.onchange = function() {
                if(this.checked) {
                    ...
                    priceSpan.innerHTML = products[productIndex].Summ
                    highPriceSpan.innerHTML = products[productIndex].HighSumm
...
zakazBtn.onclick = function() {
    ...
    priceOfCheck.innerHTML = highPriceOfCheck
